...
class Admin::TenantBackupController < ApplicationController
  ...
  def delete_backup
    ...
    # Delete only files resolved inside the exports directory.
    backup_path = safe_backup_path(backup_file[:path])
  ...
  def safe_backup_path(path)
    export_dir = Rails.root.join("tmp", "exports").realpath
    candidate = Pathname.new(path).realpath
    candidate.to_s.start_with?(export_dir.to_s) ? candidate.to_s : nil
  rescue Errno::ENOENT
    nil
  end
end
