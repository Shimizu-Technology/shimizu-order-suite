...
module Admin
  ...
  private
    ...
    def user_params
      raw_params = params.to_unsafe_h.deep_symbolize_keys
      allowed_user_keys = [ :email, :first_name, :last_name, :phone, :role, :restaurant_id, :password ]

      # Handle both direct parameters and nested user parameters while still
      # explicitly slicing to an allowlist.
      user_attributes = raw_params[:user].is_a?(Hash) ? raw_params[:user].slice(*allowed_user_keys) : {}

      # Merge with direct parameters (direct parameters take precedence)
      permitted = raw_params.slice(:email, :first_name, :last_name, :phone, :restaurant_id, :password)
      permitted.merge!(user_attributes.select { |k, _v| permitted[k].nil? })
