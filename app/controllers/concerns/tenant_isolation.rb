...
module TenantIsolation
  ...
  def set_current_tenant
    ...
    # Step 2: Set tenant context for the request duration.
    #
    # Access validation must happen after authentication. Controller callbacks from
    # this concern run before each controller's authorize_request callback, so
    # validating here would treat authenticated users as public users and make
    # production-only public-origin patches decide access. authorize_request now
    # validates the resolved tenant after current_user is available.
