...
class TenantBackupService
  class << self
    ...
    def export_tenant(restaurant, options = {})
      ...
      # Use argument-form system calls to avoid shell interpolation.
      system("zip", "-r", archive_path.to_s, ".", chdir: export_dir.to_s)
    ...
    def import_tenant(archive_path, options = {})
      ...
      # Extract the archive without invoking a shell.
      system("unzip", "-q", archive_path.to_s, "-d", import_dir.to_s)
    ...
    def validate_backup(archive_path)
      ...
      begin
        # Extract the archive without invoking a shell.
        system("unzip", "-q", archive_path.to_s, "-d", validation_dir.to_s)
    ...
    def migrate_tenant(export_path, target_env, options = {})
      ...
      # HTTP migration above is the supported path. Avoid legacy shell-based
      # scp/ssh migration because it is hard to secure and duplicated the work.
      true
    end
