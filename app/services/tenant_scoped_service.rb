...
class TenantScopedService
  ...
  def scope_query(query)
    ...
    when "Option"
      ...
      base_query.joins(option_group: { menu_item: :menu }).where(menus: { restaurant_id: @restaurant.id })
    when "OptionGroup"
      # OptionGroups belong to MenuItems which belong to Menus which belong to Restaurants
      base_query.joins(menu_item: :menu).where(menus: { restaurant_id: @restaurant.id })
    when "Category"
      # Categories belong to Menus which belong to Restaurants
      base_query.where(menu_id: Menu.where(restaurant_id: @restaurant.id).pluck(:id))
    when "VipCodeRecipient"
      # VipCodeRecipients belong to VipAccessCodes which belong to Restaurants
      base_query.joins(:vip_access_code).where(vip_access_codes: { restaurant_id: @restaurant.id })
    when "StoreCredit"
      # StoreCredits belong to Users which belong to Restaurants
      base_query.joins(:user).where(users: { restaurant_id: @restaurant.id })
    when "SeatSection"
    ...
    when "SeatAllocation"
      ...
      base_query.joins(seat: :seat_section).where(seat_sections: { restaurant_id: @restaurant.id })
    when "Seat"
      # Seats belong to SeatSections which belong to Restaurants
      base_query.joins(:seat_section).where(seat_sections: { restaurant_id: @restaurant.id })
    when "OrderPayment"
      # OrderPayments belong to Orders which belong to Restaurants
      base_query.joins(:order).where(orders: { restaurant_id: @restaurant.id })
    ...
    when "MerchandiseVariant"
      ...
      base_query.joins(merchandise_item: :merchandise_collection).where(merchandise_collections: { restaurant_id: @restaurant.id })
    when "MerchandiseItem"
      # MerchandiseItems belong to MerchandiseCollections which belong to Restaurants
      base_query.joins(:merchandise_collection).where(merchandise_collections: { restaurant_id: @restaurant.id })
    ...
    when "Restaurant"
      ...
      base_query.where(id: @restaurant.id)
    else
