class OrderChannel < ApplicationCable::Channel
  def subscribed
    Rails.logger.info("OrderChannel subscription attempt - " + {
    ...
    if restaurant_id.present?
      ...
      begin
        ...
        Rails.logger.info("OrderChannel subscription successful - " + {
        ...
        Rails.logger.error("OrderChannel stream creation failed - " + {
          channel: channel_name,
          user_id: current_user.id,
          error: e.message,
      ...
      Rails.logger.error("OrderChannel subscription rejected - " + {
  ...
  def unsubscribed
    Rails.logger.info("OrderChannel unsubscribed - " + {
      user_id: current_user&.id,
      restaurant_id: restaurant_id,
      connection_id: connection.connection_identifier,
      subscription_id: identifier,
