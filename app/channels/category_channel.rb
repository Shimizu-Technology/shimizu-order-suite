class CategoryChannel < ApplicationCable::Channel
  def subscribed
    Rails.logger.info("CategoryChannel subscription attempt - " + {
    ...
    if restaurant_id.present?
      ...
      begin
        ...
        Rails.logger.info("CategoryChannel subscription successful - " + {
        ...
        Rails.logger.error("CategoryChannel stream creation failed - " + {
          channel: channel_name,
          user_id: current_user.id,
          error: e.message,
      ...
      Rails.logger.error("CategoryChannel subscription rejected - " + {
  ...
  def unsubscribed
    Rails.logger.info("CategoryChannel unsubscribed - " + {
      user_id: current_user&.id,
      restaurant_id: restaurant_id,
      connection_id: connection.connection_identifier,
      subscription_id: identifier,
