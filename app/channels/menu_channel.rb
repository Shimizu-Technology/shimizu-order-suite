class MenuChannel < ApplicationCable::Channel
  def subscribed
    Rails.logger.info("MenuChannel subscription attempt - " + {
    ...
    if restaurant_id.present?
      ...
      begin
        ...
        Rails.logger.info("MenuChannel subscription successful - " + {
        ...
        Rails.logger.error("MenuChannel stream creation failed - " + {
          channel: channel_name,
          user_id: current_user.id,
          error: e.message,
      ...
      Rails.logger.error("MenuChannel subscription rejected - " + {
  ...
  def unsubscribed
    Rails.logger.info("MenuChannel unsubscribed - " + {
      user_id: current_user&.id,
      restaurant_id: restaurant_id,
      connection_id: connection.connection_identifier,
      subscription_id: identifier,
